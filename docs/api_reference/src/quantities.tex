...
struct @\exposidnc{unit-interface}@ {
  ...
  {
    if constexpr (std::is_same_v<M, std::remove_const_t<decltype(mag<1>)>>)
      return u;
    else
...
} hertz;  // $\unit{Hz}$ can't measure becquerel, activity,
          // or any other quantity with dimension $\unit{T}^{-1}$
...
to encapsulate a common conversion factor between units\irefiev{112-01-33}
of the operands to the addition of quantities.
\begin{example}
The result of \tcode{1 * km + 1 * mi}
has a common unit $[8/125] \unit{m}$
...
consteval bool @\exposidnc{is-one}@(std::meta::info type_alias) {
  return dealias(type_alias) == ^one;
}

template<@\exposconceptnc{DerivedUnitExpr}@... Expr>
struct @\libglobal{derived_unit}@ : @\exposidnc{unit-interface}@, @\exposidnc{expr-fractions}@<@\exposidnc{is-one}@, Expr...> {
